...
class GenererBulletinPDFAPIView(APIView):
    ...
    def get(self, request, candidature_id):
        ...
        INDIGO_LIGHT = colors.HexColor("#3b82f6")
        ...
        s_brand = style("brand", fontSize=26, textColor=WHITE, fontName="Helvetica-Bold", alignment=TA_LEFT)
        s_tagline = style("tagline", fontSize=9, textColor=colors.HexColor("#93c5fd"), fontName="Helvetica", alignment=TA_LEFT)
        s_doc_title = style("doc_title", fontSize=13, textColor=WHITE, fontName="Helvetica-Bold", alignment=TA_RIGHT)
        s_doc_sub = style("doc_sub", fontSize=8, textColor=colors.HexColor("#93c5fd"), fontName="Helvetica", alignment=TA_RIGHT)
        s_ref = style("ref", fontSize=8, textColor=SLATE_LIGHT, fontName="Helvetica", alignment=TA_RIGHT)
        ...
        s_stamp = style("stamp", fontSize=11, textColor=EMERALD, fontName="Helvetica-Bold", alignment=TA_CENTER)
        s_center = style("center", fontSize=9, textColor=SLATE_LIGHT, fontName="Helvetica", alignment=TA_CENTER)
        ...
        logo_cell = Image(logo_path, width=28*mm, height=14*mm) if os.path.exists(logo_path) else Paragraph("TAFTECH", s_brand)

        header_data = [
            [
                logo_cell,
                Paragraph(
                    "BULLETIN DE PRÉSENTATION OFFICIEL<br/>"
                    f"<font size='8' color='#93c5fd'>Réf : {ref_num}</font>",
            ...
            [
                ...
                Paragraph(
                    f"<font size='8' color='#93c5fd'>{timezone.now().strftime('%d/%m/%Y')}</font>",
        ...
        amber_table = Table(amber_data, colWidths=[170*mm])
        ...
        story.append(Paragraph(
            "La plateforme <b>TafTech</b> certifie par le présent bulletin que le candidat "
            "mentionné ci-dessous a été sélectionné et retenu à l'issue du processus de "
            "recrutement intelligent conduit sur notre plateforme.",
        ...
        cand_data = [
            [Paragraph("Nom complet", s_label), Paragraph(nom, s_value),
             Paragraph("Titre professionnel", s_label), Paragraph(titre, s_value)],
            [Paragraph("Email", s_label), Paragraph(email, s_body),
             Paragraph("Téléphone", s_label), Paragraph(telephone, s_body)],
            [Paragraph("Wilaya", s_label), Paragraph(wilaya, s_body),
             Paragraph("Niveau d'études", s_label), Paragraph(diplome, s_body)],
        ]
        cand_table = Table(cand_data, colWidths=[35*mm, 50*mm, 35*mm, 50*mm])
        ...
        poste_data = [
            [Paragraph("Intitulé du poste", s_label), Paragraph(candidature.offre.titre, s_value),
             Paragraph("Type de contrat", s_label), Paragraph(candidature.offre.type_contrat or "—", s_value)],
            [Paragraph("Entreprise", s_label), Paragraph(entreprise.nom_entreprise, s_value),
             Paragraph("Secteur d'activité", s_label), Paragraph(entreprise.secteur_activite or "—", s_body)],
            [Paragraph("Localisation", s_label), Paragraph(candidature.offre.wilaya or "—", s_body),
             Paragraph("Salaire proposé", s_label), Paragraph(candidature.offre.salaire_propose or "À négocier", s_body)],
        ]
        poste_table = Table(poste_data, colWidths=[35*mm, 50*mm, 35*mm, 50*mm])
    ...
    # bloc score IA
            ...
            score_data = [[
                Paragraph(
                    f'<b><font color="{score_color}">{score:.0f}%</font></b> de compatibilité avec le poste',
                    style("score_txt", fontSize=12, textColor=SLATE, fontName="Helvetica", leading=18)
                ),
                Paragraph(
                    "Score calculé par l'algorithme de matching TafTech basé sur la spécialité, "
                    "le diplôme, l'expérience, la localisation et les compétences clés.",
                    style("score_desc", fontSize=8, textColor=SLATE_LIGHT, fontName="Helvetica", leading=12)
                ),
            ]]
            score_table = Table(score_data, colWidths=[45*mm, 125*mm])
        ...
        cert_data = [[
            Paragraph(
                "Ce bulletin a été généré automatiquement par la plateforme <b>TafTech</b> "
                "et certifie que le candidat a été sélectionné via notre processus de "
                "recrutement intelligent. Ce document est valide sans signature manuscrite.",
        ...
        cert_table = Table(cert_data, colWidths=[120*mm, 50*mm])
        ...
        sig_data = [[
            Paragraph(
                ...
                "<font color='#64748b' size='8'>Plateforme de recrutement intelligente<br/>"
                "Oran, Algérie — taftech963@gmail.com</font>",
                style("sig_left", fontSize=10, textColor=SLATE, fontName="Helvetica", leading=16)
            ),
            Paragraph(
                "<b>Signature électronique</b><br/>"
                f"<font color='#64748b' size='8'>Réf: {ref_num}<br/>"
                f"Émis le {timezone.now().strftime('%d/%m/%Y à %H:%M')}</font>",
        ...
        story.append(Paragraph(
            "TAFTECH — Plateforme de recrutement intelligente en Algérie | "
            "taftech963@gmail.com | Oran, Algérie | "
